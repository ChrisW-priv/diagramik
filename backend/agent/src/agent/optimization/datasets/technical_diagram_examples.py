...
def get_technical_diagram_examples() -> list[dspy.Example]:
    """Get 17 technical diagram examples (14 train / 3 dev).
    ...
    examples_data = [
        ...
        # 16. GCP monolith (from failed conversation - agent should NOT generate imports)
        {
            "user_request": "standard GCP arch of monolith: CloudSQL + bucket + CR service",
            "expected_node_names": ["CloudSQL", "GCS", "CloudRun"],
        },
        # 17. Self-correction: agent made import mistake, now corrects
        {
            "conversation_history": (
                "## User\nstandard GCP arch of monolith: CloudSQL + bucket + CR service\n\n"
                "## Assistant\n[Called draw_technical_diagram with code containing import statements]\n\n"
                "## Tool Error\nError: code must not contain import statements. "
                "All nodes are pre-imported. Just use node constructors directly."
            ),
            "user_request": "standard GCP arch of monolith: CloudSQL + bucket + CR service",
            "expected_node_names": ["CloudSQL", "GCS", "CloudRun"],
        },
