...
1. **Layout and spacing**: Establish the spatial rhythm and visual hierarchy.
1. **Typography and color**: Apply the type scale and color system.
1. **Interactive states**: Hover, focus, active, disabled.
1. **Edge case states**: Empty, loading, error, overflow, first-run.
1. **Motion**: Purposeful transitions and animations (if appropriate).
1. **Responsive**: Adapt for different viewports. Don't just shrink; redesign for the context.
...
1. **Does it pass the AI slop test?** If someone saw this and said "AI made this," would they believe it immediately? If yes, it needs more design intention.
1. **Check against impeccable's DON'T guidelines.** Fix any anti-pattern violations.
1. **Check every state.** Navigate through empty, error, loading, and edge case states. Each one should feel intentional, not like an afterthought.
1. **Check responsive.** Resize the viewport. Does it adapt well or just shrink?
1. **Check the details.** Spacing consistency, type hierarchy clarity, color contrast, interactive feedback, motion timing.
